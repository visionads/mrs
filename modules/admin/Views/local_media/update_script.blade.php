<script>
    ...
    $(document).on("focus",'#update-table tr:last-child td:last-child',function(e) {
        ...
        if(coa_name != "")
        {
            ...
            var element = '<tr>\
            <td>\
            <div>{!! Form::text('title_option[]', Input::old('title'), ['title'=>'Enter Title', 'class' => 'form-control']) !!}</div>\
            </td>\
            <td>\
            <div>{!! Form::text('price[]', Input::old('price'), ['title'=>'Enter Price', 'class' => 'form-control']) !!}</div>\
            </td>\
            </tr>';
